...
`filename` / `file_name`, `directory` / `folder`, normalized language shorthands such as `rs`, `ts`, `py`, `js`, `cpp`, `cs`, `sh`, `bash`, and `md`, and `exclude_*` aliases such as `exclude_folder`, `exclude_lang`, `exclude_ext`, `exclude_kind`,
