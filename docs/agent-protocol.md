...
Use `find_symbol`, `find_index_symbol`, or `find_shard_symbol` when the next step is a direct definition jump. These tools accept the same path, file, language, extension, test, dependency/import, symbol, and kind filters as search, so agents can keep a narrowed scope instead of broadening a symbol lookup across the whole repo set.
