...
Use `find_symbol`, `find_index_symbol`, or `find_shard_symbol` when the next step is a direct definition jump. Use their `_batch` variants when the agent has several candidate names from a search or repo map. These tools accept the same path, file, language, extension, test, dependency/import, symbol, and kind filters as search, so agents can keep a narrowed scope instead of broadening a symbol lookup across the whole repo set.
