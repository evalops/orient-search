...
`code:true` / `is:code` matches implementation source-code languages such as Rust, Python, TypeScript, JavaScript, Go, Ruby, Java, Kotlin, Swift, C, C++, and C# while excluding docs/config formats like Markdown, TOML, JSON, YAML, and text; `code:false` / `is:docs` does the inverse for prose/config searches.
...
`filename` / `file_name`, `directory` / `folder`, normalized language shorthands such as `rs`, `ts`, `py`, `js`, `cpp`, `cs`, and `md`, and `exclude_*` aliases such as `exclude_folder`, `exclude_lang`, `exclude_ext`, `exclude_kind`,
