...
`interface`/`interfaces`, `target`/`recipe`/`task` for Makefile and Justfile
targets, `script` / `package-script` for package.json and pyproject scripts,
`package` for Cargo and pyproject package names, and `bin` / `example` /
`bench` for Cargo manifest entries.
