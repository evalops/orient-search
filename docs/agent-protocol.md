...
4. Use `related_request` when the likely next step is finding nearby tests, source counterparts, or sibling files for a hit; returned related files include `read_request` payloads for opening the file directly, and `include_read_batch:true` asks Orient to include one batch read request for all returned ranges.
5. Use `related_symbols_request` when the likely next step is finding nearby definitions, types, or other symbols for a hit; search-generated requests already carry the original query, returned related symbols include `read_request` payloads for the next bounded read, and `include_read_batch:true` asks Orient to include one batch read request for all returned definitions.
