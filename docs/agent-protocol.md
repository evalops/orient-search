...
Bare single-token filename and path-like queries such as `Cargo.toml`, `README.md`, or `src/lib.rs` are inferred as `file:` / `path:` filters so agents that type the file they want get the file, not references to its name. Pytest node IDs such as `tests/test_auth.py::test_login` are inferred as path filters too, including simple commands like `pytest tests/test_auth.py::test_login -q` or `python -m pytest tests/test_auth.py::test_login`. Simple Rust test commands such as `cargo test parser_accepts_locations` and `cargo test query::tests::parser_accepts_locations` infer `kind:function symbol:parser_accepts_locations`. Use `content:Cargo.toml`, `text:README.md`, or `term:src/lib.rs` when the literal string is the target.
