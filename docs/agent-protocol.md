...
`daemon_status`, or the direct CLI wrapper `orient daemon-status`, reports warmed index and shard status so multiple local agents can confirm they share the intended codebase set. The default CLI output is compact; use `orient daemon-status --format json` for full warmed-target details and `search_auto_default`, which tells wrappers whether no-target `search_auto` will use a single warmed shard directory, a single warmed index, or live fallback search. It also includes `default_requests`: copyable JSON-lines requests for `tool_manifest`, `agent_guide`, the right repo-map surface, no-target `search_auto`, `search_auto_batch`, and the right query-plan surface. Each item includes `jsonl` and `client_cli` fields so shell-native agents can pipe the next call directly into `orient client-jsonl`. `serve-tcp`, `serve-unix`, and the `daemon-status --addr/--socket --format json` CLI rewrite `client_cli` pipes in returned payloads to target the actual daemon address or socket, including ordinary search/read/related follow-up responses. When exactly one index or shard directory is warmed, indexed and shard tools marked with `daemon_default.source` may omit `index` or `index_dir`; if zero or multiple targets are warmed, pass the path explicitly. `search_auto` and `search_auto_batch` can also run without a target: explicit `index_dir`, `index`, or `repo` wins first, then one warmed daemon target, then live fallback search from the daemon runtime. Orient does not expose session analytics.
