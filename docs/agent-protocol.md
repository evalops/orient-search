...
Bazel labels such as `//tools/search:orient_cli` and `:agent_smoke_test` infer `kind:target` plus the label's target name; package labels also add a path filter for the package portion.
