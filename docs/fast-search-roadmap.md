...
- Direct symbol lookup and related-context lookup from live and persistent indexes, including filter-aware `find_symbol`/`find_index_symbol`/`find_shard_symbol` tools and test-to-source stem matching for common `_test`, `test_`, `.test`, and `.spec` naming, so agent wrappers can jump to definitions and nearby tests/files without rebuilding a repo index.
