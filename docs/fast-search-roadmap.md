...
- Indexed search stores compressed exact-symbol posting lists and uses them to narrow identifier-shaped raw queries such as `SessionManager` or `agent_instructions`, so agents get symbol-like speed without having to remember `symbol:`.
