...
- JSON-lines search tools accept structured query-filter aliases such as `lang`, `ext`, `kind`, `type`, `dep`, and `module`, plus structured `exclude_*` filters as strings or arrays, so wrappers can express positive and negative filters without query-string rewriting.
