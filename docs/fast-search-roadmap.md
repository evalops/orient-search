...
- `orient index-shards`, `orient search-shards`, and `orient read-shard-range`: local multi-repo shard manifest with one versioned index file per repo and bounded range reads from prefixed shard paths.
...
- JSON-lines tools: `search_code`, `indexed_search_code`, `index_shards`, `search_shards`, `read_shard_range`, `repo_map`, `read_range`, and `related_symbols`.
...
- Wide tree fallback: `/Users/jonathanhaas/Documents/Projects`, common top-10 literal/token queries at `19-37ms` p95 after warmup across the sampled runs.
...
- Multi-repo shard directories store a manifest plus one versioned index per repo.
