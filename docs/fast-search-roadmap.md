...
- Indexed search stores compressed exact-symbol posting lists and uses them to narrow explicit `symbol:` filters plus identifier-shaped raw queries such as `SessionManager` or `agent_instructions`, so agents get symbol-like speed whether they use structured filters or plain names.
