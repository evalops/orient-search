...
- `orient bench-search` and `orient bench-shards`: built-in p50/p95/max latency reporting for fallback, indexed, and shard search, with `--fail-p95-ms`, `--write-baseline`, and `--baseline` for regression gates.
...
- Wide tree fallback: `/Users/jonathanhaas/Documents/Projects`, common top-10 literal/token queries at about `21-38ms` p95 after warmup across the latest sampled release run.
...
- Local single-shard search: query `repo:agent-jsonl-explorer indexed search symbol filters`, top 10 at about `2.18ms` p95 after warmup.
