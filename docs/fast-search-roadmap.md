...
- Search results include structured `line_range` metadata, exact `match_lines` from indexed token-to-line tables when available, a compact `read_range` hint, and a ready-to-send `read_request` with the correct follow-up tool for live, indexed, or shard searches.
