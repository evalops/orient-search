...
- Indexed search stores compressed exact-symbol and symbol-kind posting lists, using them to narrow explicit `symbol:` / `kind:` filters plus identifier-shaped raw queries such as `SessionManager` or `agent_instructions`, so agents get symbol-like speed whether they use structured filters or plain names.
