...
- Live fallback pushes safe positive and negative file/path scope filters plus conservative `generated:false` directory excludes down into `rg`/`fd` before Orient's Rust matcher rechecks candidates, so common agent scopes like `path:src -path:generated` or `generated:false` avoid walking avoidable generated trees.
