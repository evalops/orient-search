...
- Wide tree fallback: `/Users/jonathanhaas/Documents/Projects`, common top-10 literal/token queries at `18-41ms` p95 after warmup across the sampled runs.
...
- Tests cover fallback search, indexed search, incremental refresh, filters, ranking, duplicate suppression, JSON-lines server calls, corrupt index errors, path safety, snippet modes, and a guarded `rg` differential check.
