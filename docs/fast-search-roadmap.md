...
- `index_status` and `shard_status` report live-file freshness versus persisted indexes, including added, changed, and deleted paths; indexed and shard searches can opt into `refresh_if_stale` for a one-call refresh-before-search path. Index, shard, and daemon status outputs expose footprint counters including source bytes, posting entries, and compressed posting bytes.
